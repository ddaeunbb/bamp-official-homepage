export const BAMP_LINK = {
  mail: 'bamp_41th@gmail.com',
  instagram: 'https://www.instagram.com/gobamp/',
  youtube: 'https://www.youtube.com/@bamp3754',
  cafe: 'https://cafe.naver.com/gobamp',
  blog: 'https://blog.naver.com/12bamp',
  apply: 'https://blog.naver.com/12bamp/224370119167',
} as const;

export const YOUTUBE_LINK = {
  '41': 'https://youtu.be/xPK5HEkbpA0',
  '40': 'https://youtu.be/Prj07UeKu50',
  '39': 'https://youtu.be/k0Qy2ueprEM',
  '38': 'https://www.youtube.com/watch?v=mjQDY5tHLjs',
  '37-1': 'https://youtu.be/Nzk0P5ijxbY?si=q9dlVwj735EQD2D2',
  '37': 'https://youtube.com/shorts/LLWSRnjxnk8?si=WOwE3lEx10m6_LV2',
  '36': 'https://youtu.be/AMPxNG6WOmY?si=F709J03HmiDU8cMn',
  '35': 'https://youtu.be/_rDHC3L8isU?si=9r2iE3IJkcO6LQkz',
  '34': 'https://youtu.be/aDJOK9WoMMQ?si=mWU7pcJZp2ud6HvX',
  '33': 'https://youtu.be/dfUfOPfqRJQ?si=m-zPvEUmQLn-0dKR',
  '32': 'https://youtu.be/7cHgf71U1C8?si=ySgKC9BE0lySwQwo',
  '31': 'https://youtu.be/Qa14CcKyL1w?si=Ps53u69ByFibFlek',
  '30': 'https://youtu.be/wqFYRgrXkJw?si=xLptS2q3haWjo54t',
} as const;
